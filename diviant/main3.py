...
def analyze_video_for_activity(uploaded_video, activity):
    ...
    def process_video_chunks(video_chunks, activity):
        ...
        for chunk, start_time, end_time in video_chunks:
            ...
            for i in range(len(activities_list)):
                ...
                results_probs.append(
                    (current_activity, f"Probability: {prob * 100:.2f}%")
                )
    ...
    if best_result["likely_label"] != "other":
        base_name = os.path.basename(uploaded_video)
        name, ext = os.path.splitext(base_name)
        output_filename = f"{name}_best_segment.mp4"
        ...
        print(
            f"{datetime.now()} - The best video segment has been saved as '{output_filename}'"
        )
    else:
        print(
            f"{datetime.now()} - The most likely activity is 'other'. No segment was saved."
        )
...
@app.get("/main3")
async def root2(video_path: str, activity: str):
    # Example usage
    # video_path = "/content/Shoplifting037_x264.mp4"  # Replace with your video path
    # activity = "robbery, stealing, pilferage, pilfering, filching, shoplifting, thievery, purloining, embezzlement, hijacking, larceny, Colloq lifting, appropriation, pocketing, pinching, swiping, snitching, Chiefly Brit nicking, US boosting, Slang heist, knocking off, rip-off"  # Replace with your activity label

    best_result = analyze_video_for_activity(video_path, activity)

    # Save and print results
    best_result["concatenated_image"].save("best_concatenated_Shoplifting037_x264.jpg")
    print(f"{datetime.now()} - Best Probability:")
    for activity, prob in best_result["results_probs"]:
        print(f"{datetime.now()} - {activity}: {prob}")

    print(f"{datetime.now()} - \nBest Raw Scores:")
    for activity, score in best_result["results_logits"]:
        print(f"{datetime.now()} - {activity}: {score}")

    print(
        f"{datetime.now()} - \nMost Likely Activity: {best_result['likely_label']} with Probability: {best_result['likely_probability']:.2f}%"
    )
